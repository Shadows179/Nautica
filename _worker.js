...
// Variables - jangan hardcode sensitive data
const rootDomain = "pages.dev";
const serviceName = "babagan";
...
// Pindahkan kelas CloudflareApi ke atas sebelum digunakan
class CloudflareApi {
  constructor() {
...
class ProxyConverter {
  static toClash(proxies) {
    ...
    proxies.forEach((proxy, index) => {
      ...
      if (proxyInfo) {
        clashConfig.proxies.push(proxyInfo);
        clashConfig["proxy-groups"][0].proxies.push(proxyInfo.name);
      }
    });

    return this.toYAML(clashConfig);
  }

  static toSurfboard(proxies) {
    let surfboardConfig = "[General]\n";
    surfboardConfig += "loglevel = notify\n";
    surfboardConfig += "interface = 127.0.0.1\n";
    surfboardConfig += "port = 8888\n\n";
    surfboardConfig += "[Proxy]\n";

    proxies.forEach((proxy, index) => {
      const proxyInfo = this.parseProxy(proxy);
      if (proxyInfo) {
        surfboardConfig += `${proxyInfo.name} = ${proxyInfo.type}, ${proxyInfo.server}, ${proxyInfo.port}`;
        if (proxyInfo.password) surfboardConfig += `, password=${proxyInfo.password}`;
        if (proxyInfo.cipher) surfboardConfig += `, encrypt-method=${proxyInfo.cipher}`;
        if (proxyInfo.uuid) surfboardConfig += `, uuid=${proxyInfo.uuid}`;
        surfboardConfig += "\n";
      }
    });

    return surfboardConfig;
  }

  static parseProxy(proxyUrl) {
    try {
      const url = new URL(proxyUrl);
      const protocol = url.protocol.replace(':', '');
      
      let proxyInfo = {
        name: `Proxy-${Math.random().toString(36).substring(2, 8)}`,
        type: protocol,
        server: url.hostname,
      ...
      if (url.username) {
        if (protocol === 'ss') {
          try {
            const auth = atob(url.username).split(':');
            proxyInfo.cipher = auth[0];
            proxyInfo.password = auth[1];
          } catch (e) {
            proxyInfo.password = url.username;
          }
        } else {
          proxyInfo.uuid = url.username;
        }
      }
